...
def solution(sizes, limits, tasks):
    ...
    for size, limit, task in zip(sizes, limits, tasks):
        ...
        for char in str(task):
            if char.isdigit():  # 숫자인지 아닌지 확인
                # 숫자라면, 글자를 정수로 변환하여 해당 작업을 수행하는데 필요한 시간을 계산하고 total_time에 더함
                total_time += calculate_time(size, int(char))
        if total_time <= limit:  # 주어진 시간 내에 작업을 끝낼 수 있는지 확인
            result.append(1) # 작업 완료할 수 있다면 result에 1 추가
        else:
            result.append(0) # 그렇지 않다면 result에 0을 추가
    return result # result 리스트를 반환하여 각 로봇의 작업 완료 여부를 나타내는 결과를 얻기